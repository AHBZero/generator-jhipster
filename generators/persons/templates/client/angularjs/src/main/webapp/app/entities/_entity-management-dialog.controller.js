...
(function() {
    ...
    function <%= entityAngularName %>ManageController ($stateParams, previousState, $state<% if (fieldsContainOwnerOneToOne) { %>, $q<% } %><% if (fieldsContainBlob) { %>, DataUtils<% } %>, entity, <%= entityClass %><% for (idx in differentTypes) { if (differentTypes[idx] !== entityClass) {%>, <%= differentTypes[idx] %><% } } %>) {
                ...
                if (relationships[idx].selectize) {
                  if (relationships[idx].relationshipType === 'many-to-many' && relationships[idx].ownerSide === true) {
                      var prepareArray = 'vm.' + relationships[idx].relationshipFieldNamePlural.toLowerCase() + ' = [];\n'
                        + '        if (vm.'+ entityInstance +'.'+ relationships[idx].relationshipFieldNamePlural +' !== undefined) {\n'
                        + '            vm.' + relationships[idx].relationshipFieldNamePlural.toLowerCase() + ' = vm.' + relationships[idx].relationshipFieldNamePlural.toLowerCase() + '.concat(vm.' + entityInstance + '.'+ relationships[idx].relationshipFieldNamePlural + ');\n'
                        + '        } else { \n'
                        + '             vm.' + entityInstance + '.'+ relationships[idx].relationshipFieldNamePlural + ' = []\n'
                        + '        } \n';
        ...
        vm.<%=relationships[idx].relationshipFieldNamePlural%>Selected = function ($id, $model) {
            console.log($id, $model);
            <% if (relationships[idx].relationshipType === 'many-to-many' && relationships[idx].ownerSide === true) { %>
            vm.<%= entityInstance %>.<%= relationships[idx].relationshipFieldNamePlural %> = $model;
            <% } else { %>
